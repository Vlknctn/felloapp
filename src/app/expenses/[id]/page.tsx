    ...
    <div className="flex flex-col min-h-full" style={{ backgroundColor: "var(--bg-base)" }}>
      ...
      <div className="flex-1 flex flex-col items-center pt-4 pb-12 px-4">
        ...
        <div className="w-full max-w-[340px] relative">
          {/* Top notch — Always white receipt regardless of theme */}
        ...
        {/* ── CogniMemo Insight ── */}
        <div className="w-full max-w-[340px] mt-8 inline-prompt inline-prompt--positive cursor-pointer hover:opacity-90 transition-opacity"
             style={{ borderRadius: "var(--radius-xl)", padding: "16px" }}>
          <div className="w-9 h-9 shrink-0 rounded-[11px] flex items-center justify-center" style={{ backgroundColor: "var(--accent-muted)", color: "var(--accent-text)" }}>
            <Brain className="w-4 h-4" />
          </div>
          <div className="flex-1">
            <h4 className="font-bold text-[13px] mb-1" style={{ color: "var(--accent-text)" }}>CogniMemo Yorumu</h4>
            ...
            <button className="mt-2 text-[12px] font-semibold flex items-center gap-1 hover:gap-2 transition-all" style={{ color: "var(--accent-text)" }}>
              Tüm Hafızayı Gör <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
        </div>
      </div>
